...
const Answer: React.FC<AnswerProps> = props => {
  return (
    <li key={props.key} onClick={() => props.clickItem(props.text)}>
      {props.text}
    </li>
  );
};
...
const Quiz: React.FC<QuizProps> = props => {
  ...
  return (
    <section>
      ...
      {isAnswer && (
        <CorrectAnswer is_correct={isCorrect}>
          {data.quiz[pageNumber].answer[data.quiz[pageNumber].correct]}
        </CorrectAnswer>
      )}
      {data.quiz.length - 1 !== pageNumber && (
